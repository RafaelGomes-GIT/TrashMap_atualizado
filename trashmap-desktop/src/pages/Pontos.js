...
    try {
      // Faz a chamada para Geocodificação Reversa (location)
      const { results, status } = await geocoder.geocode({ location: { lat: latLng.lat, lng: latLng.lng } });

      if (status === 'OK' && results[0]) {
        const addressComponents = results[0].address_components;
        
        // Usa a função auxiliar para extrair os campos do formulário
        const parsedAddress = parseAddressComponents(addressComponents);

        // ATUALIZA TODOS OS CAMPOS do formCadastro!
        setFormCadastro(prev => ({ 
          ...prev, 
          ...parsedAddress, // logradouro, numero, bairro, cidade, estado, cep
          latitude: lat,
          longitude: lng 
        }));
...
      } else {
        alert('Geocodificação Reversa falhou. Status: ' + status);
      }
    } catch (err) {
      alert('Erro na comunicação com a API de Geocoding Reversa: ' + err.message);
    }
  }
